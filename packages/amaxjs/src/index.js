...
function createEos(config) {
  ...
  abis.push(abiCache.abi("amax.null", eosio_null));
  abis.push(abiCache.abi("amax.token", token));
  abis.push(abiCache.abi("amax", system));
  ...
  if (config.mockTransactions != null) {
    ...
    assert.equal(
      typeof config.mockTransactions,
      "function",
...
function mergeWriteFunctions(config, EosApi, structs, abis) {
  ...
  throwOnDuplicate(
    merge,
    writeApi,
    "Conflicting methods in EosApi and Transaction Api"
  );
...
*/
const defaultSignProvider = (eos, config) =>
  async function ({ sign, buf, transaction, optionsKeyProvider }) {
    // optionsKeyProvider is a per-action key: await eos.someAction('user2' .., {keyProvider: privateKey2})
    const keyProvider = optionsKeyProvider
      ? optionsKeyProvider
      : config.keyProvider;

    if (!keyProvider) {
      throw new TypeError(
        "This transaction requires a keyProvider for signing"
      );
    }
    ...
    return eos
      .getRequiredKeys(transaction, pubkeys)
      .then(({ required_keys }) => {
        if (!required_keys.length) {
          throw new Error(
            "missing required keys for " + JSON.stringify(transaction)
          );
        }
        ...
        if (missingKeys.length !== 0) {
          ...
          keyProvider({ pubkeys: missingKeys }).forEach((pvt) => {
            pvts.push(pvt);
          });
        }
