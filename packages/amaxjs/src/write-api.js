...
function writeApiGen(Network, network, structs, config, abis) {
  ...
  */
  merge.contract = (...args) => {
    const { params, options, returnPromise, callback } = processArgs(
      args,
      ["account"],
      "contract",
      optionsFormatter
...
function WriteApi(Network, network, config, Transaction) {
  ...
  */
  const genTransaction = (structs, merge) =>
    async function (...args) {
      ...
      if (isContractArray) {
        ...
        const cachedCode = new Set(["amax", "amax.token", "amax.null"]);
      ...
      assert.equal(
        args.length,
        1,
        "transaction args: contracts<string|array>, transaction<callback|object>, [options], [callback]"
      );
      const arg = args[0];
      ...
      if (contracts) {
        ...
        assert.equal(
          "function",
          typeof arg,
          "provide function callback following contracts array parameter"
        );
  ...
  function genContractActions(account, transaction = null) {
    return config.abiCache.abiAsync(account).then((cache) => {
      assert(
        Array.isArray(cache.abi.actions) && cache.abi.actions.length,
        "No actions"
      );

      const contractMerge = {};
      contractMerge.transaction = transaction
        ? transaction
  ...
  function genMethod(
    ...
    return function (...args) {
      if (args.length === 0) {
        console.log(
          usage({ name, type }, definition, Network, account, config)
        );
        return;
      }
      ...
      if (
        ...
        typeof lastArg.__optionOverrides === "object"
      ) {
      ...
      const optionDefaults = {
        // From config and configDefaults
  ...
  */
    ...
    const wrap =
      ...
      (...args) => {
        ...
        if (ret == null) {
          ...
          throw new Error(
            "Callbacks can not be used when creating a multi-action transaction"
          );
        }
  ...
  async function transaction(arg, options, callback) {
    const defaultExpiration = config.expireInSeconds
    ...
    if (typeof arg !== "object") {
      throw new TypeError(
        "First transaction argument should be an object or function"
      );
    }
    ...
    if (options.sign && typeof config.signProvider !== "function") {
      throw new TypeError(
        "Expecting config.signProvider function (disable using {sign: false})"
      );
    }
    ...
    if (config.transactionHeaders) {
      if (typeof config.transactionHeaders === "object") {
        ...
        await config.transactionHeaders(
          ...
          checkError(callback, config.logger, async function (headers) {
            assert.equal(
              typeof headers,
              "object",
              "expecting transaction header object"
            );
        ...
        assert(
          false,
          "config.transactionHeaders should be an object or function"
        );
      }
    }
    ...
    assert.equal(
      typeof rawTx.expiration,
      "string",
      "expecting expiration: iso date time string"
    );
    assert.equal(
      typeof rawTx.ref_block_num,
      "number",
      "expecting ref_block_num number"
    );
    assert.equal(
      typeof rawTx.ref_block_prefix,
      "number",
      "expecting ref_block_prefix number"
    );
    ...
    Promise.all(sigs)
      .then((sigs) => {
        ...
        if (mock != null) {
          assert(
            /pass|fail/.test(mock),
            "mockTransactions should return a string: pass or fail"
          );
          if (mock === "pass") {
            callback(null, {
          ...
          if (mock === "fail") {
            const error = `[push_transaction mock error] 'fake error', digest '${buf.toString(
              "hex"
            )}'`;
...
function usage(action, definition, Network, account, config) {
  ...
  out("EXAMPLE");
  out(
    `${account}.${action.name}(${JSON.stringify(struct.toObject(), null, 4)})`
  );
