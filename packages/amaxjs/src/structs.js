...
module.exports = (config = {}, extendedSchema) => {
  ...
  const forceActionDataHex =
    config.forceActionDataHex != null ? config.forceActionDataHex : true;
  ...
  const { structs, types, errors, fromBuffer, toBuffer } = Fcbuffer(
    fullSchema,
    config
  );
...
const variant =
  (...variantArray) =>
  (validation, baseTypes, customTypes) => {
    const variants = variantArray.map((Type) =>
      Type(validation, baseTypes, customTypes)
    );
...
const PublicKeyEcc = (validation) => {
  return {
    ...
    appendByteBuffer(b, value) {
      ...
      // }
      const buf = PublicKey.fromStringOrThrow(
        value,
        validation.keyPrefix
      ).toBuffer();
    ...
    toObject(value) {
      if (validation.defaults && value == null) {
        const keyPrefix = validation.keyPrefix ? validation.keyPrefix : "AM";
...
const ExtendedSymbol = (validation, baseTypes, customTypes) => {
  ...
  return {
    ...
    appendByteBuffer(b, value) {
      ...
      assert(
        contract != null,
        "Missing @contract suffix in extended symbol: " + value
      );
  ...
  return {
    ...
    appendByteBuffer(b, value) {
      assert.equal(
        typeof value,
        "object",
        "expecting extended_asset object, got " + typeof value
      );
    ...
    fromObject(value) {
      ...
      if (typeof value === "string") {
        ...
        assert(
          false,
          "expecting extended_asset<object|string>, got: " + typeof value
        );
      }
  ...
  "setcode.code.fromObject": ({ object, result }) => {
    try {
      ...
      if (/^\s*\(module/.test(code)) {
        ...
        assert(
          binaryen != null,
          'required: config.binaryen = require("binaryen")'
        );
        if (config.debug) {
...
const actionDataOverride = (structLookup, forceActionDataHex) => ({
  "action.data.fromByteBuffer": ({ fields, object, b, config }) => {
    const ser =
      (object.name || "") == ""
        ? fields.data
        : structLookup(object.name, object.account);
    if (ser) {
    ...
    if (ser) {
      ...
      const data =
        typeof object.data === "string"
          ? Buffer.from(object.data, "hex")
          : object.data;
      if (!Buffer.isBuffer(data)) {
        throw new TypeError(
          `Unknown struct '${object.name}' for contract '${object.account}', locate this struct or provide serialized action.data`
        );
      }
