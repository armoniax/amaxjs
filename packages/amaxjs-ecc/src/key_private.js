...
function parseKey(privateStr) {
  ...
  const privateKey = PrivateKey.fromBuffer(
    keyUtils.checkDecode(keyString, keyType)
  );
...
/**
    ...
    @arg {string} privateStr AMAXio or Wallet Import Format (wif) -- a secret
*/
...
function unitTest() {
  ...
  assert.equal(
    pub.toString(),
    "AM859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhToVM",
    pubError
  );
  // assert.equal(pub.toString(), 'PUB_K1_859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2Ht7beeX', pubError)
  // assert.equal(pub.toStringLegacy(), 'AM859gxfnXyUriMgUeThh1fWv3oqcpLFyHa3TfFYC4PK2HqhToVM', pubError)

  doesNotThrow(
    () => PrivateKey.fromString(pvt.toWif()),
    "converting known wif from string"
  );
  doesNotThrow(
    () => PrivateKey.fromString(pvt.toString()),
    "converting known pvt from string"
  );
  doesNotThrow(
    () => PublicKey.fromString(pub.toString()),
